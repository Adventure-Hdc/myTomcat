...
public class SessionManager {
    ...
    public static HttpSession getSession(String id, RequestBIO requestBIO, Response response) {
        if(id == null) {
            //浏览器没有传递sessionID
            return newSession(requestBIO, response);
        } else {
            ...
            if(currentSession == null) {
                ...
                creatCookieBySession(currentSession, requestBIO, response);
                return currentSession;
            }
        }
    }

    private static HttpSession newSession(RequestBIO requestBIO, Response response) {
        ServletContext servletContext = requestBIO.getServletContext();
        ...
        creatCookieBySession(session, requestBIO, response);
        return session;
    }

    private static void creatCookieBySession(StandardSession session, RequestBIO requestBIO, Response response) {
        Cookie cookie = new Cookie("JSESSIONID", session.getId());
        cookie.setMaxAge(session.getMaxInactiveInterval());
        cookie.setPath(requestBIO.getContext().getPath());
